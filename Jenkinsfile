        ...
        stage('ssh docker pull') {
            ...
            steps {
                ...
                sshagent(['jenkins-deploy']) {
                    sh "ssh -o StrictHostKeyChecking=no root@10.41.152.227 docker ps -q --filter name=${params.projectName} && docker rm -f \$(docker ps -aq --filter name=${params.projectName})"
