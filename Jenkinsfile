        ...
        stage('ssh docker pull & run') {
            ...
            steps {
                ...
                sshagent(['jenkins-deploy']) {
                    ...
                    sh "ssh -o StrictHostKeyChecking=no root@10.41.152.227 docker ps -q --filter name=${params.projectName} | grep -q . && docker rm -f \$(docker ps -aq --filter name=${params.projectName}) || echo Not Found"
                    sh "ssh -o StrictHostKeyChecking=no root@10.41.152.227 docker rmi \$(docker images --filter 'dangling=true' -q --no-trunc)"
