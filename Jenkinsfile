        ...
        stage('ssh docker pull & run') {
            ...
            steps {
                ...
                sshagent(['jenkins-deploy']) {
                    ...
                    sh "ssh -o StrictHostKeyChecking=no root@10.41.152.227 docker ps -qa --filter name=${params.projectName} | grep -q . && docker rm -f ${params.projectName} || echo Not Found"
                    sh "ssh -o StrictHostKeyChecking=no root@10.41.152.227 docker images --filter 'dangling=true' -q --no-trunc | grep -q . && docker rmi `docker images --filter 'dangling=true' -q --no-trunc` || echo Not Found Image"
